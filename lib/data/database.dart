...
class DBProvider {
  ...
  Future<Database> initDB() async {
    ...
    return await openDatabase(path, version: 1, onOpen: (db) {},
      ...
      await db.execute(
          "CREATE TABLE subjects (id INTEGER PRIMARY KEY, title TEXT, teacher TEXT)");
  ...
  Future<List<Timetable>> getTimetable({int weekday}) async {
    ...
    var res = await db
        .query('shedule', where: 'weekday = ?', whereArgs: [weekday - 1]);
    List<Timetable> list = res.isNotEmpty
        ? res.map((data) => Timetable.fromMap(data)).toList()
        : [];
    return list;
  }
  ...
  Future timetable(Timetable timetable) async {
    ...
    if (timetable.id == null) {
      return await db.rawInsert(
          "INSERT INTO shedule (weekday, subject, start, end) VALUES (?, ?, ?, ?)",
          [
            timetable.weekday - 1,
            timetable.subject,
            timetable.start,
    ...
    return await db.update('shedule', timetable.toMap(),
  ...
  Future deleteTimetable({int id, int weekday}) async {
    final db = await database;
    var homeworks =
        await db.query('homeworks', where: "idShedule = ?", whereArgs: [id]);
  ...
  Future<List<Homework>> getNotDoneHomeworks() async {
    final db = await database;
    var res =
        await db.query('homeworks', where: 'isDone = 0 AND content != ""');
    List<Homework> list = res.isNotEmpty
        ? res.map((data) => Homework.fromMap(data)).toList()
        : [];
    return list;
  }
  ...
  Future<List<Homework>> getHomeWorks(int date, int common) async {
    final db = await database;
    var res = await db.query("homeworks",
        where: 'date = ? AND common = ?', whereArgs: [date, common]);
    List<Homework> list = res.isNotEmpty
        ? res.map((data) => Homework.fromMap(data)).toList()
        : [];
    return list;
  ...
  Future<Homework> getHomework({int date, int subject, int idTimetable}) async {
    final db = await database;
    var res = await db.query("homeworks",
        where: 'date = ? AND subject = ? AND idShedule = ?',
        whereArgs: [date, subject, idTimetable]);
    Homework homework = res.isNotEmpty
        ? res.map((data) => Homework.fromMap(data)).toList()[0]
        : Homework();
    return homework;
  }
  ...
  Future homework(Homework homework) async {
    ...
    if (homework.id == null) {
      int id = 1;
      var lastHomework =
          await db.rawQuery("SELECT * FROM homeworks ORDER BY id DESC LIMIT 1");
    ...
    return await db.update('homeworks', homework.toMap(),
  ...
  Future<List<Subject>> getSubjects() async {
    ...
    List<Subject> list =
        res.isNotEmpty ? res.map((data) => Subject.fromMap(data)).toList() : [];
  ...
  Future<Map<String, String>> subject(Subject subject) async {
    final db = await database;
    List subjects = await db
        .query('subjects', where: 'title = ?', whereArgs: [subject.title]);
  ...
  Future deleteSubject(int id) async {
    ...
    var timetable =
        await db.query('shedule', where: "subject = ?", whereArgs: [id]);
    if (timetable.isNotEmpty) {
      for (var k = 0; k < timetable.length; k++) {
        await DBProvider.db
            .deleteTimetable(id: Homework.fromMap(timetable[k]).id);
      }
    }
  ...
  Future<List<Homework>> getGrades(int subject) async {
    final db = await database;
    var res =
        await db.query('homeworks', where: 'subject = ?', whereArgs: [subject]);
    List<Homework> list = res.isNotEmpty
        ? res.map((data) => Homework.fromMap(data)).toList()
        : [];
    return list;
  ...
  Future importDB() async {
    ...
    if (permissionResult) {
      await close();
      final directory =
          join(await ExtStorage.getExternalStorageDirectory(), 'school');
